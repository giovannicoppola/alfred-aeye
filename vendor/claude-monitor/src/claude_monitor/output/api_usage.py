...
def _utilization_to_pct(value: Any) -> Optional[float]:
    """Normalize Anthropic ``utilization`` to a 0–100 percentage.

    The OAuth usage API returns utilization as a percentage in ``[0, 100]``
    (see Claude-Code-Usage-Monitor #202), not a 0–1 fraction. Values in
    ``(0, 1]`` are therefore real percentages like 1%, not 100%.
    """
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        return None
...
def _window(raw: Any, now_epoch: Optional[int]) -> Optional[Dict[str, Any]]:
    ...
    if "percent" in raw and utilization is None:
...
def _from_limits_array(
    payload: Dict[str, Any], now_epoch: Optional[int]
) -> Optional[Dict[str, Optional[Dict[str, Any]]]]:
    """Prefer the structured ``limits[]`` list when Anthropic includes it."""
    ...
    for item in items:
        if not isinstance(item, dict):
            continue
        kind = item.get("kind")
        window = _window(item, now_epoch)
        if window is None or window.get("used_percentage") is None:
            # Still accept resets-only windows
            if window is None:
                continue
        if kind == "session":
            five = window
...
def _normalize(
    ...
    from_list = _from_limits_array(payload, now_epoch)
    if from_list is not None:
        five = from_list.get("five_hour")
        seven = from_list.get("seven_day")
    else:
        limits = (
        ...
        five = _window(limits.get("five_hour"), now_epoch)
        seven = _window(limits.get("seven_day"), now_epoch)
    return {
        "five_hour": five,
        "seven_day": seven,
